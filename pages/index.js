...
function App() {
  ...
  return (
    <div className={styles.wrapper}>
      <div className={styles.header}>Decentralized Task Management system</div>
      <div className={styles.container}>
        <form className={styles.form} onSubmit={handleSubmit}>
          <label htmlFor="task" className={styles.label}>Add Task:</label>
          ...
          <tbody>
            {tasks.map((task, index) => (
              <tr key={index}>
                ...
                <td>
                  {task.status === 0 ? (
                    <button className={styles.button} onClick={() => changeTaskStatus(index)}>
                      Mark as Finished
                    </button>
                  ) : null}
                </td>
